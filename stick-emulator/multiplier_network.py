...
class MultiplierNetwork(SpikingNetworkModule):
    def __init__(self, encoder: DataEncoder, prefix: str = '') -> None:
        ...
        self.acc_log1 = ExplicitNeuron(Vt, tm, tf, neuron_id=prefix + "acc_log1")

        self.first2 = ExplicitNeuron(Vt, tm, tf, neuron_id=prefix + "first2")
        self.last2 = ExplicitNeuron(Vt, tm, tf, neuron_id=prefix + "last2")
        self.acc_log2 = ExplicitNeuron(Vt, tm, tf, neuron_id=prefix + "acc_log2")
        ...
        self.add_neurons([
            self.input1, self.input2, self.first1, self.last1, self.acc_log1,
            self.first2, self.last2, self.acc_log2, self.sync, self.acc_exp, self.output
        ])
...
from multiplier_network import MultiplierNetwork  # replace with your filename if needed
