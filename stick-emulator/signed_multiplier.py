...
from multiplier_network import MultiplierNetwork
...
class SignedMultiplierNetwork(SpikingNetworkModule):
    def __init__(self, encoder: DataEncoder, prefix: str = '') -> None:
        ...
        self.input1_plus = ExplicitNeuron(Vt, tm, tf, neuron_id=prefix + "input1_plus")
        self.input1_minus = ExplicitNeuron(Vt, tm, tf, neuron_id=prefix + "input1_minus")
        self.input2_plus = ExplicitNeuron(Vt, tm, tf, neuron_id=prefix + "input2_plus")
        self.input2_minus = ExplicitNeuron(Vt, tm, tf, neuron_id=prefix + "input2_minus")

        self.output_plus = ExplicitNeuron(Vt, tm, tf, neuron_id=prefix + "output_plus")
        self.output_minus = ExplicitNeuron(Vt, tm, tf, neuron_id=prefix + "output_minus")
        ...
        self.add_neurons([ 
            self.input1_plus, self.input1_minus,
            self.input2_plus, self.input2_minus,
            self.output_plus, self.output_minus,
            self.sign1, self.sign2, self.sign3, self.sign4
        ])
