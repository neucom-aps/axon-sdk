from primitives import ExplicitNeuron, EventQueue, SpikingNetworkModule, DataEncoder
from subtractor_network import SubtractorNetwork
from synchronizer_network import SynchronizerNetwork
import math

class LinearCombinationNetwork(SpikingNetworkModule):
    def __init__(self, encoder: DataEncoder, N: int, coeff: list, prefix: str = ""):
        ...
        self.acc1_plus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=prefix + "_acc1_plus")
        self.acc1_minus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=prefix + "_acc1_minus")
        self.acc2_plus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=prefix + "_acc2_plus")
        self.acc2_minus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=prefix + "_acc2_minus")
        self.inter_minus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=prefix + "_inter_minus")
        self.inter_plus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=prefix + "_inter_plus")
        self.output_plus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=prefix + "_output_plus")
        self.output_minus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=prefix + "_output_minus")
        self.start = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=prefix + "_start")
        self.sync = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=prefix + "_sync")
        ...
        self.connect_neurons(self.sync_network.output_neurons[0], self.subtractor_network.input1, 'V', we, Tsyn)
        self.connect_neurons(self.sync_network.output_neurons[1], self.subtractor_network.input2, 'V', we, Tsyn)
        # connect outputs (plus/minus) to start and output +/- neurons
        self.connect_neurons(self.subtractor_network.output_plus, self.start, 'V', we, Tsyn)
        self.connect_neurons(self.subtractor_network.output_minus, self.start, 'V', we, Tsyn)
        self.connect_neurons(self.subtractor_network.output_plus, self.output_plus, 'V', we, Tsyn)
        self.connect_neurons(self.subtractor_network.output_minus, self.output_minus, 'V', we, Tsyn)
        # Recurrent connection to start
        self.connect_neurons(self.start, self.start, 'V', wi, Tsyn)

        # connect Inter+ to input0 of synchronizer
        self.connect_neurons(self.inter_plus, self.sync_network.input_neurons[0], 'V', we, Tsyn)
        # connect Inter- to input1 of synchronizer
        self.connect_neurons(self.inter_minus, self.sync_network.input_neurons[1], 'V', we, Tsyn)
        ...
        # Add these neurons
        self.add_neurons([
            self.acc1_plus, self.acc1_minus,
            self.acc2_plus, self.acc2_minus,
            self.inter_minus, self.inter_plus,
            self.output_plus, self.output_minus,
            self.start, self.sync
        ])
        ...
        for i in range(N):
            ...
            input_plus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=f"input_plus_{i}")
            input_minus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=f"input_minus_{i}")
            # Create first and last +/- 
            first_plus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=f"first_plus_{i}")
            first_minus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=f"first_minus_{i}")
            last_plus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=f"last_plus_{i}")
            last_minus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=f"last_minus_{i}")

            self.add_neurons([input_plus, input_minus, first_plus, first_minus, last_plus, last_minus])
            ...
            self.connect_neurons(first_plus, self.acc1_plus, 'ge', a_i * wacc, Tsyn + Tmin)
            ...
            self.connect_neurons(first_minus, self.acc1_minus, 'ge', a_i * wacc, Tsyn + Tmin)
...
def run_test(coeffs, inputs, Tmin=10.0, Tcod=100.0):
    ...
    N = len(coeffs)
    net = LinearCombinationNetwork(encoder, N=N, coeff=coeffs)
    ...
    if decoded_plus is not None:
        print(f"🟢 output+ interval = {plus_spikes[1] - plus_spikes[0]:.3f} ms → decoded = {decoded_plus:.3f}")
    else:
        print("🔴 output+ did not spike twice.")

    if decoded_minus is not None:
        print(f"🟢 output- interval = {minus_spikes[1] - minus_spikes[0]:.3f} ms → decoded = -{decoded_minus:.3f}")
    else:
        print("🔴 output- did not spike twice.")
