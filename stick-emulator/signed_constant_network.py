...
class SignedConstantNetwork(SpikingNetworkModule):
    def __init__(self, encoder: DataEncoder, value: float, prefix: str = '') -> None:
        ...
        self.recall = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, Vreset=0.0, neuron_id=prefix +'recall')
        self.output_plus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, Vreset=0.0, neuron_id=prefix+'output_plus')
        self.output_minus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, Vreset=0.0, neuron_id=prefix+'output_minus')
...
if __name__ == '__main__':
    ...
    output_spikes = sim.spike_log[constant_network.output_plus.id]
