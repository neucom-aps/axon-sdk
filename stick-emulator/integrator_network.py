...
from signed_constant_network import SignedConstantNetwork
from linear_comb_network import LinearCombinationNetwork    



class IntegratorNetwork(SpikingNetworkModule):
    def __init__(self, encoder: DataEncoder, constant: float, coeffs: list, prefix: str= '') -> None:
        super().__init__()
        ...
        self.input_plus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=prefix + "input_plus")
        self.input_minus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=prefix +"input_minus")
        self.start = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=prefix +"start")
        self.output_plus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=prefix +"output_plus")
        self.output_minus = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=prefix +"output_minus")
        self.new_input = ExplicitNeuron(Vt=Vt, tm=tm, tf=tf, neuron_id=prefix +"new_input")

        self.constant_network = SignedConstantNetwork(encoder, constant, prefix + 'sc')
        self.lin_comb = LinearCombinationNetwork(encoder, 2, coeffs, prefix=prefix +"lin_comb")
        ...
        self.connect_neurons(self.input_plus, self.lin_comb.input_neurons[2], 'V', we, Tsyn)
        self.connect_neurons(self.input_minus, self.lin_comb.input_neurons[3], 'V', we, Tsyn)

        self.connect_neurons(self.start, self.lin_comb.input_neurons[2], 'V', we, Tsyn)
        self.connect_neurons(self.start, self.lin_comb.input_neurons[2], 'V', we, Tsyn + Tmin)

        self.connect_neurons(self.constant_network.output_plus, self.lin_comb.input_neurons[0], 'V', we, Tsyn)
        self.connect_neurons(self.constant_network.output_minus, self.lin_comb.input_neurons[1], 'V', we, Tsyn)

        self.connect_neurons(self.lin_comb.output_plus, self.lin_comb.input_neurons[0], 'V', we, Tsyn)
        self.connect_neurons(self.lin_comb.output_minus, self.lin_comb.input_neurons[1], 'V', we, Tsyn)

        self.connect_neurons(self.lin_comb.output_plus, self.output_plus, 'V', we, Tsyn)
        self.connect_neurons(self.lin_comb.output_minus, self.output_minus, 'V', we, Tsyn)
